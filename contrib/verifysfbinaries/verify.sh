...
###   This script attempts to download the signature file SHA256SUMS.asc from collegicoin.org
...
WORKINGDIR="/tmp/collegicoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://collegicoin.org/bin/"
VERSIONPREFIX="collegicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'collegicoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Collegicoin Core binary release signing key installed?"
   fi
