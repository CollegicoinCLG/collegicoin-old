...
const std::string CollegicoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString CollegicoinGUI::DEFAULT_WALLET = "~Default";

CollegicoinGUI::CollegicoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Collegicoin") + " - ";
#ifdef ENABLE_WALLET
    ...
    statusbar->setStyleSheet("QFrame { background-color: #961c1f; }");
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #961c1f; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #00CCFF, stop: 1 #33CCFF); border-radius: 7px; margin: 0px; }");
    }
    ...
    QPixmap pixmapLogo(":/images/light/collegicoin_logo_horizontal");
...
void CollegicoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Collegicoin address"));
    ...
    aboutAction = new QAction(tr("&About Collegicoin"), this);
    aboutAction->setStatusTip(tr("Show information about Collegicoin"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Collegicoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Collegicoin addresses"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the Collegicoin help message to get a list with possible Collegicoin command-line options"));
...
void CollegicoinGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void CollegicoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool CollegicoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool CollegicoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void CollegicoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CollegicoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void CollegicoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Collegicoin client") + " " + networkStyle->getTitleAddText();
...
void CollegicoinGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void CollegicoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CollegicoinGUI::optionsClicked()
{
...
void CollegicoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void CollegicoinGUI::showDebugWindow()
{
...
void CollegicoinGUI::showInfo()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_INFO);
}

void CollegicoinGUI::showConsole()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_CONSOLE);
}

void CollegicoinGUI::showGraph()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_GRAPH);
}

void CollegicoinGUI::showPeers()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_PEERS);
}

void CollegicoinGUI::showRepair()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_REPAIR);
}

void CollegicoinGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void CollegicoinGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void CollegicoinGUI::showBackups()
{
    GUIUtil::showBackups();
}

void CollegicoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void CollegicoinGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
#ifdef ENABLE_WALLET
void CollegicoinGUI::openClicked()
{
...
void CollegicoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CollegicoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CollegicoinGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void CollegicoinGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void CollegicoinGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void CollegicoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CollegicoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CollegicoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CollegicoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void CollegicoinGUI::updateNetworkState()
{
...
void CollegicoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CollegicoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void CollegicoinGUI::updateHeadersSyncProgressLabel()
{
...
void CollegicoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in collegicoin
        {
...
void CollegicoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void CollegicoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Collegicoin"); // default title
...
void CollegicoinGUI::changeEvent(QEvent *e)
{
...
void CollegicoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CollegicoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CollegicoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(CollegicoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void CollegicoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CollegicoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CollegicoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CollegicoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CollegicoinGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void CollegicoinGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void CollegicoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CollegicoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CollegicoinGUI::detectShutdown()
{
...
void CollegicoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void CollegicoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void CollegicoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CollegicoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void CollegicoinGUI::subscribeToCoreSignals()
{
...
void CollegicoinGUI::unsubscribeFromCoreSignals()
{
...
void CollegicoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void CollegicoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<CollegicoinUnits::Unit> units = CollegicoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const CollegicoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(CollegicoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(CollegicoinUnits::Unit u, CollegicoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(CollegicoinUnits::name(u)), this);
...
{
    setText(CollegicoinUnits::name(newUnits));
}
