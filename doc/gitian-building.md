...
*Setup instructions for a Gitian build of Collegicoin Core using a Debian VM or physical system.*
...
- [Building Collegicoin Core](#building-collegicoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-collegicoin-host/README.md).
...
Clone the git repositories for Collegicoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/collegicoindev/collegicoin
```
...
in the Collegicoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build Collegicoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Collegicoin Core repository.
...
```bash
    ...
    From https://github.com/collegicoindev/collegicoin
...
For example:
```bash
URL=https://github.com/crowning-/collegicoin.git
...
and the collegicoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../collegicoin/contrib/gitian-descriptors/*|sort|uniq )
...
CLGPATH=/some/root/path/collegicoin.git
SIGPATH=/some/root/path/collegicoin-detached-sigs.git

./bin/gbuild --url collegicoin=${CLGPATH},signature=${SIGPATH} ../collegicoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/collegicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/collegicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/collegicoin-osx-build.assert
```
...
[collegicoin/gitian.sigs](https://github.com/motioncrypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
